  ...
  const tour =
    open && mounted
      ? createPortal(
          ...
          >
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-[var(--border)] bg-[var(--bg)] px-4 py-3 lg:px-8">
              <button
                type="button"
                onClick={() => setOpenIndex(null)}
                className="inline-flex items-center gap-2 rounded-full px-3 py-2 text-sm font-semibold text-[var(--ink)] transition hover:bg-[var(--surface-tint)]"
                aria-label="Close photo gallery"
              >
                <IconClose className="h-5 w-5" />
                Close
              </button>
              <p id={titleId} className="text-sm font-medium text-[var(--ink-secondary)]">
    ...
    <>
      <div className="flex flex-col gap-3">
        <button
          ...
          disabled={!hero}
          className="group relative h-[400px] overflow-hidden rounded-[var(--radius-md)] bg-[var(--surface-tint)] text-left disabled:cursor-default"
        ...
        >
          ...
          {images.length > 1 ? (
            <span className="pointer-events-none absolute bottom-4 right-4 rounded-full bg-[var(--bg)]/95 px-3.5 py-2 text-sm font-semibold text-[var(--ink)] shadow-sm ring-1 ring-[var(--border)]">
              Show all photos
            </span>
          ) : null}
        </button>
              ...
              return (
                <li key={`${src}-${index}`} className="shrink-0">
                  <button
                    ...
                    className="h-20 w-20 overflow-hidden rounded-[var(--radius-md)] bg-[var(--surface-tint)] ring-offset-2 transition hover:ring-2 hover:ring-[var(--accent)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]"
